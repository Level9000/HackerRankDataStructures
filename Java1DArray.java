 ...
 * Each element in the collection is accessed using an index, and the elements 
 * are easy to find because they're stored sequentially in memory.
 * 
 ...
 * 
 * This sets aside a block of memory that's capable of storing 4 integers. Each integer storage cell is 
 * assigned a unique index ranging from 0 to one less than the size of the array, and each cell initially contains a 0. 
 * In the case of myArray, we can store integers at indices 0, 1, 2, and 3. Let's say we wanted the 
 ...
 * 
 * The code above prints the value stored at index 3 of myArray, which is 
 * 12 (the value we previously stored there). It's important to note that while Java initializes each cell 
 * of an array of integers with a 0, not all languages do this.
 * 
 * Task
 * 
 ...
 *       location in the array. For example, the first value must be stored in a0, the second value must be stored in a1, and so on.
 * Good luck!
 */
